...
def label_regimes(df: pd.DataFrame, model: GaussianHMM) -> pd.DataFrame:
    """
    Label the HMM states as Bullish, Neutral, or Bearish based on
    cumulative returns in each state (better for trend detection).
    
    Also labels volatility as Low, Medium, or High based on
    average volatility in each state.
    """
    df = df.copy()
    
    # Find the volatility column (may vary based on LONG_WINDOW setting)
    vol_col = None
    for col in df.columns:
        if col.startswith('rolling_std_') and col != f'rolling_std_{SHORT_WINDOW}':
            vol_col = col
            break
    if vol_col is None:
        vol_col = f'rolling_std_{LONG_WINDOW}'
    
    # Find cumulative return column
    cum_ret_col = None
    for col in df.columns:
        if col.startswith('cum_return_'):
            cum_ret_col = col
            break
    if cum_ret_col is None:
        cum_ret_col = f'cum_return_{LONG_WINDOW}'
    
    # Calculate statistics for each state
    agg_dict = {'Return': ['mean', 'std']}
    if vol_col in df.columns:
        agg_dict[vol_col] = 'mean'
    if cum_ret_col in df.columns:
        agg_dict[cum_ret_col] = 'mean'
    
    state_stats = df.groupby('HMM_State').agg(agg_dict).round(6)
    
    # Flatten column names
    state_stats.columns = ['_'.join(col).strip() for col in state_stats.columns]
    
    # Use cumulative return for trend labeling (better than daily return mean)
    if f'{cum_ret_col}_mean' in state_stats.columns:
        trend_col = f'{cum_ret_col}_mean'
    else:
        trend_col = 'Return_mean'
    
    sorted_by_return = state_stats[trend_col].sort_values()
    
    # Handle case where we might have fewer than 3 states
    n_states = len(sorted_by_return)
    if n_states >= 3:
        trend_mapping = {
            sorted_by_return.index[0]: 'bearish',   # Lowest return
            sorted_by_return.index[1]: 'neutral',   # Middle return
            sorted_by_return.index[2]: 'bullish'    # Highest return
        }
    elif n_states == 2:
        trend_mapping = {
            sorted_by_return.index[0]: 'bearish',
            sorted_by_return.index[1]: 'bullish'
        }
    else:
        trend_mapping = {sorted_by_return.index[0]: 'neutral'}
    
    # Sort states by average volatility to assign volatility labels
    if f'{vol_col}_mean' in state_stats.columns:
        sorted_by_vol = state_stats[f'{vol_col}_mean'].sort_values()
        
        if n_states >= 3:
            vol_mapping = {
                sorted_by_vol.index[0]: 'low',
                sorted_by_vol.index[1]: 'medium',
                sorted_by_vol.index[2]: 'high'
            }
        elif n_states == 2:
            vol_mapping = {
                sorted_by_vol.index[0]: 'low',
                sorted_by_vol.index[1]: 'high'
            }
        else:
            vol_mapping = {sorted_by_vol.index[0]: 'medium'}
    else:
        vol_mapping = {i: 'medium' for i in range(n_states)}
    
    # Apply labels
    df['Trend_Regime'] = df['HMM_State'].map(trend_mapping)
    df['Volatility_Regime'] = df['HMM_State'].map(vol_mapping)
    
    # Store state statistics for later use
    df.attrs['state_stats'] = state_stats
    df.attrs['trend_mapping'] = trend_mapping
    df.attrs['vol_mapping'] = vol_mapping
...
# =============================================================================
# VALIDATION: KNOWN MARKET EVENTS
# =============================================================================

# Define known bearish periods for validation
KNOWN_BEARISH_EVENTS = {
    '2008_financial_crisis': {
        'name': '2008 Financial Crisis',
        'start': '2008-09-01',
        'end': '2009-03-31',
        'description': 'Lehman collapse, S&P500 dropped ~50%',
        'severity': 'extreme'
    },
    '2020_covid_crash': {
        ...
        'start': '2020-02-20',
        'end': '2020-03-23',
        'description': 'Pandemic crash, fastest 30% drop in history',
        'severity': 'extreme'
    },
    '2022_bear_market': {
        ...
        'start': '2022-01-01',
        'end': '2022-10-15',
        'description': 'Inflation/Fed tightening, S&P500 dropped ~25%',
        'severity': 'moderate'
    },
    '2018_q4_selloff': {
        ...
        'start': '2018-10-01',
        'end': '2018-12-24',
        'description': 'Fed tightening fears, ~20% correction',
        'severity': 'moderate'
    },
    '2011_debt_ceiling': {
        ...
        'start': '2011-07-01',
        'end': '2011-10-01',
        'description': 'US debt downgrade, ~19% correction',
        'severity': 'moderate'
    },
    '2015_china_crash': {
        ...
        'start': '2015-08-01',
        'end': '2016-02-15',
        'description': 'China devaluation, oil crash, ~15% correction',
        'severity': 'moderate'
    }
}

# Define known bullish periods for validation
KNOWN_BULLISH_EVENTS = {
    '2009_recovery': {
        'name': '2009 Recovery Rally',
        'start': '2009-03-10',
        'end': '2009-12-31',
        'description': 'Post-crisis recovery, S&P500 up ~65%',
        'severity': 'strong'
    },
    '2020_recovery': {
        ...
        'start': '2020-03-24',
        'end': '2020-12-31',
        'description': 'Post-COVID rally, historic stimulus',
        'severity': 'strong'
    },
    '2017_low_vol_bull': {
        'name': '2017 Low-Vol Bull Run',
        'start': '2017-01-01',
        'end': '2017-12-31',
        'description': 'One of the calmest bull years ever',
        'severity': 'strong'
    },
    '2021_bull_market': {
        ...
        'start': '2021-01-01',
        'end': '2021-12-31',
        'description': 'Continued post-COVID rally',
        'severity': 'strong'
    },
    '2023_recovery': {
        'name': '2023 Recovery',
        'start': '2023-01-01',
        'end': '2023-12-31',
        'description': 'Recovery from 2022 bear market',
        'severity': 'moderate'
    },
    '2024_bull_market': {
        ...
        'start': '2024-01-01',
        'end': '2024-12-31',
        'description': 'AI-driven rally, S&P500 up ~25%',
        'severity': 'strong'
    }
}


def validate_event_detection(df: pd.DataFrame, 
                             event: dict, 
                             expected_regime: str) -> dict:
    """
    Validate whether the HMM correctly identified a known market event.
    
    Args:
        df: DataFrame with Trend_Regime column populated
        event: Dictionary with 'start', 'end', 'name' keys
        expected_regime: 'bearish' or 'bullish'
    ...
    event_data = df.loc[mask].dropna(subset=['Trend_Regime'])
    ...
    if len(event_data) == 0:
        return {
            ...
            'status': 'NO_DATA',
            'message': f"No data available for {event['name']} ({event['start']} to {event['end']})",
            'accuracy': 0,
            'days_total': 0,
            'days_correct': 0
        }
    
    # Count days in expected regime
    days_correct = (event_data['Trend_Regime'] == expected_regime).sum()
    days_total = len(event_data)
    accuracy = days_correct / days_total * 100
    
    # Determine pass/fail - realistic thresholds
    # Markets are messy, even a good model won't be perfect
    # 50% for extreme (must catch the big ones), 35% for moderate
    threshold = 50 if event.get('severity') == 'extreme' else 35
    passed = accuracy >= threshold
    
    # Get regime breakdown
    regime_counts = event_data['Trend_Regime'].value_counts().to_dict()
...
def validate_regime_detection(df: pd.DataFrame) -> dict:
    """
    Run comprehensive validation of regime detection against known market events.
    
    Args:
        df: DataFrame with Trend_Regime column populated
    
    Returns:
        Dictionary with validation results for all events
    """
    results = {
        'bearish_events': [],
        'bullish_events': [],
    ...
    # Validate bearish events
    for event_key, event in KNOWN_BEARISH_EVENTS.items():
        result = validate_event_detection(df, event, 'bearish')
        results['bearish_events'].append(result)
    
    # Validate bullish events
    for event_key, event in KNOWN_BULLISH_EVENTS.items():
        result = validate_event_detection(df, event, 'bullish')
        results['bullish_events'].append(result)
    
    # Calculate summary statistics
    all_events = results['bearish_events'] + results['bullish_events']
    events_with_data = [e for e in all_events if e['status'] != 'NO_DATA']
    
    if events_with_data:
        passed = sum(1 for e in events_with_data if e['status'] == 'PASS')
        failed = sum(1 for e in events_with_data if e['status'] == 'FAIL')
        avg_accuracy = sum(e['accuracy'] for e in events_with_data) / len(events_with_data)
        
        results['summary'] = {
            'total_events': len(events_with_data),
            'passed': passed,
            'failed': failed,
            'pass_rate': round(passed / len(events_with_data) * 100, 1),
            'avg_accuracy': round(avg_accuracy, 1),
            'grade': get_validation_grade(passed / len(events_with_data) * 100)
        }
    else:
        results['summary'] = {
            'total_events': 0,
...
def get_validation_grade(pass_rate: float) -> str:
    """Convert pass rate to letter grade."""
    if pass_rate >= 90:
        return 'A'
    elif pass_rate >= 80:
        return 'B'
...
def print_validation_report(results: dict) -> str:
    ...
    Args:
        results: Output from validate_regime_detection()
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("HMM REGIME DETECTION VALIDATION REPORT")
    lines.append("=" * 70)
    lines.append("")
    ...
    lines.append(f"Pass Rate: {summary.get('pass_rate', 0)}% ({summary.get('passed', 0)}/{summary.get('total_events', 0)} events)")
    lines.append(f"Average Accuracy: {summary.get('avg_accuracy', 0)}%")
    lines.append("")
    
    # Bearish events
    lines.append("-" * 70)
    lines.append("BEARISH EVENT DETECTION")
    lines.append("-" * 70)
    
    for event in results['bearish_events']:
        status_emoji = "✅" if event['status'] == 'PASS' else "❌" if event['status'] == 'FAIL' else "⚠️"
        lines.append(f"\n{status_emoji} {event['event']}")
        lines.append(f"   Period: {event.get('period', 'N/A')}")
        lines.append(f"   Accuracy: {event['accuracy']}% ({event['days_correct']}/{event['days_total']} days)")
        if 'regime_breakdown' in event:
            breakdown = ', '.join([f"{k}: {v}" for k, v in event['regime_breakdown'].items()])
            lines.append(f"   Breakdown: {breakdown}")
    
    # Bullish events
    lines.append("")
    lines.append("-" * 70)
    lines.append("BULLISH EVENT DETECTION")
    lines.append("-" * 70)
    
    for event in results['bullish_events']:
        status_emoji = "✅" if event['status'] == 'PASS' else "❌" if event['status'] == 'FAIL' else "⚠️"
        lines.append(f"\n{status_emoji} {event['event']}")
        lines.append(f"   Period: {event.get('period', 'N/A')}")
        lines.append(f"   Accuracy: {event['accuracy']}% ({event['days_correct']}/{event['days_total']} days)")
...
def run_validation(df_or_ticker: pd.DataFrame = None, 
                   ticker: str = None) -> dict:
    """
    Convenience function to run full validation.
    
    Can either pass a DataFrame with regimes already detected,
    or a ticker symbol to fetch data and detect regimes.
    
    Args:
        df_or_ticker: DataFrame with Trend_Regime column, or None
        ticker: Ticker symbol to fetch and analyze
    
    Returns:
        Validation results dictionary
    """
    if df_or_ticker is not None:
        df = df_or_ticker
        if 'Trend_Regime' not in df.columns:
            # Need to run regime detection first
            result = detect_regimes(df)
            df = result['df']
    elif ticker:
        # Would need to import and fetch data
        raise NotImplementedError("Pass a DataFrame with regime detection already run")
    else:
        raise ValueError("Must provide either df_or_ticker or ticker")
    
    results = validate_regime_detection(df)
    return results
